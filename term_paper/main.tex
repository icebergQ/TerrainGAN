...
-- ``$Agreement$. The output vectors of non-faulty processes should be within Euclidean distance $\epsilon>0$, a constant deﬁned a $priori$. Formally, for any $p_i$ and $p_j$ in $G$, we have that $d(O_i,O_j) \leq \epsilon$,where $d(O_i,O_j)$ is the Euclidean distance between $O_i$ and $O_j$ in the appropriate Euclidean space."\\
